...
@section('content')
    <div class="p-4">
        <div class="bg-white rounded-xl p-6 w-full max-w-8xl relative shadow-[0_-4px_10px_rgba(0,0,0,0.08),0_6px_15px_rgba(0,0,0,0.1)]">
            ...
            <h1 class="text-center text-red-800 text-2xl font-bold mb-8">TAMBAH MAKANAN</h1>
            ...
            <form action="{{ route('menu.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nama_menu" value="{{ old('nama_menu') }}" 
                        class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-yellow-400 outline-none @error('nama_menu') border-red-500 @enderror">
                    @error('nama_menu')
                        <p class="text-red-600 text-xs mt-1 font-bold italic underline">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="harga" value="{{ old('harga') }}"
                        class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-yellow-400 outline-none @error('harga') border-red-500 @enderror">
                    @error('harga')
                        <p class="text-red-600 text-xs mt-1 font-bold italic underline">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="file" name="gambar" accept="image/*" 
                        class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-yellow-400 outline-none @error('gambar') border-red-500 @enderror">
                    <p class="text-gray-400 text-[10px] mt-1 italic">* Format: JPG, PNG, JPEG, WEBP (Maks. 2MB)</p>
                    @error('gambar')
                        <p class="text-red-600 text-xs mt-1 font-bold italic underline">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="deskripsi" rows="3" 
                        class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-yellow-400 outline-none @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi') }}</textarea>
                    @error('deskripsi')
                        <p class="text-red-600 text-xs mt-1 font-bold italic underline">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <button type="submit"
                    class="w-full bg-yellow-400 text-white font-semibold py-2 rounded-lg hover:bg-yellow-500 transition shadow-md">
                    Simpan
                </button>
            </form>
        </div>

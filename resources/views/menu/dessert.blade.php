    ...
    <div class="p-4 space-y-6 h-[calc(100vh-120px)] overflow-y-auto">
        <div
            ...
            <div class="flex justify-between items-center mb-6">
                <a href="{{ route('menu.create', 4) }}" class="bg-red-700 text-white px-4 py-2 rounded-lg hover:bg-red-800">
                    + Tambah Dessert
                </a>
...
<div id="addonModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50">
    ...
    <div class="bg-white rounded-xl p-6 w-full max-w-4xl mx-auto relative
        ...
        {{-- CLOSE --}}
        <button type="button" onclick="closeAddonModal()" class="absolute top-3 right-3 text-gray-500 hover:text-gray-700">
            ✕
        </button>

        <h1 class="text-center text-red-800 text-2xl font-bold mb-6 uppercase tracking-tight">
            ATUR OPSI MENU
        </h1>
        ...
        <form action="{{ route('menu.opsi.store') }}" method="POST">
            ...
            {{-- NAMA MENU (READONLY) --}}
            <div class="mb-4">
                <label class="font-semibold text-gray-700">Nama Menu</label>
                <input type="text" id="addonMenuNama" readonly class="w-full border rounded-lg p-2 bg-gray-100 text-gray-500 outline-none">
            </div>
            ...
            <div class="flex justify-between items-center mb-2">
                <h2 class="font-bold text-gray-800 uppercase text-sm tracking-wider">Daftar Pilihan & Harga</h2>
            </div>

            <div id="pilihanWrapper" class="space-y-3">
                <div class="flex gap-3 items-start">
                    <div class="flex-1">
                        <input type="text" name="pilihan[0][nama]" placeholder="Nama pilihan (Contoh: Extra Keju)"
                            required class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-yellow-400 outline-none">
                    </div>

                    <div class="w-40">
                        <input type="number" name="pilihan[0][harga]" placeholder="Harga (+)"
                            required min="0" class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-yellow-400 outline-none">
                    </div>
                </div>
            </div>

            {{-- Error untuk array pilihan --}}
            @if($errors->has('pilihan.*'))
                 <p class="text-red-600 text-xs font-bold mt-2 italic underline">Semua nama pilihan dan harga wajib diisi.</p>
            @endif

            <button type="button" onclick="tambahPilihan()" class="mt-4 flex items-center gap-1 text-sm font-bold text-blue-600 hover:text-blue-800 transition-colors">
            ...
            <button type="submit" class="w-full mt-8 bg-yellow-400 text-white py-3 rounded-lg font-bold hover:bg-yellow-500 shadow-md transition-all active:scale-[0.98]">
                SIMPAN OPSIONAL MENU
            </button>
        </form>
    </div>
</div>
    ...
    <div class="bg-white rounded-xl w-full max-w-3xl max-h-[90vh]
        ...
        <div class="p-6 overflow-y-auto">
            ...
            <form method="POST" action="{{ route('menu.update') }}" enctype="multipart/form-data">
                ...
                {{-- NAMA --}}
                <div class="mb-4">
                    <label class="font-semibold">Nama Dessert</label>
